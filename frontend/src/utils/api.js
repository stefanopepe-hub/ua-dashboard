/**
 * api.js v2.1 — UA Dashboard Fondazione Telethon ETS
 * Tutti gli endpoint centralizzati. safeArray garantisce sempre array.
 */
...
async function post(path, body) {
  ...
  if (!r.ok) { const text = await r.text().catch(()=>''); throw new Error(text?.slice(0,300)||`HTTP ${r.status}`) }
  return r.json()
}
...
export const api = {
  ...
  perArea:      (p={}) => get(`/kpi/saving/per-area${qs(p)}`),
  ...
  inspectFile: async (file) => {
    ...
    if (!r.ok) { const t=await r.text().catch(()=>''); throw new Error(t?.slice(0,300)||`HTTP ${r.status}`) }
    return r.json()
  },

  uploadSaving: async (file, cdcOverride) => {
    const fd = new FormData(); fd.append('file', file)
    const url = cdcOverride ? `${BASE}/upload/auto?cdc_override=${encodeURIComponent(cdcOverride)}` : `${BASE}/upload/auto`
  ...
  exportExcel: async (body) => {
    const r = await fetch(`${BASE}/export/custom/excel`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
    if (!r.ok) throw new Error(`Export failed: ${r.status}`)
    const blob = await r.blob()
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href=url; a.download=`report_${body?.filtri?.anno||'tutti'}.xlsx`; a.click()
