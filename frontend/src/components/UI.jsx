...
export function Spinner({ size = 'md' }) {
  ...
  return <div className={`animate-spin rounded-full border-2 border-telethon-blue border-t-transparent ${sz}`} />
}
...
export function KpiCard({ label, value, sub, color = 'blue', icon }) {
  const colors = { blue:'bg-telethon-blue', red:'bg-telethon-red', green:'bg-green-600', orange:'bg-orange-500', purple:'bg-purple-600', gray:'bg-gray-400' }
  return (
    <div className="kpi-card">
      <div className="flex items-start justify-between">
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</span>
        {icon && <span className={`${colors[color]} text-white rounded-lg p-1.5`}>{icon}</span>}
      </div>
...
export function Badge({ children, color = 'blue' }) {
  const cls = { blue:'bg-blue-100 text-blue-700', green:'bg-green-100 text-green-700',
    red:'bg-red-100 text-red-700', orange:'bg-orange-100 text-orange-700', gray:'bg-gray-100 text-gray-600' }[color]
  return <span className={`badge ${cls}`}>{children}</span>
}

/**
 * FilterBar con anni dinamici dal database.
 * anni = array di numeri [2026, 2025, 2024, ...]
 */
export function FilterBar({ anno, setAnno, strRic, setStrRic, cdc, setCdc, anni = [] }) {
  return (
    <div className="flex flex-wrap gap-3 mb-6">
      <select
        className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-telethon-blue font-medium"
        value={anno}
      ...
      {setStrRic && (
        <select className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-telethon-blue"
