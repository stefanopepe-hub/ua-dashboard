...
def normalize_risorse_row(
    ...
    try:
        clean_m = re.sub(r'[/\\]', '-', mese_raw.strip())
        parts = clean_m.split('-')

        # Fast-path per i formati più comuni YYYY-MM e MM-YYYY
        if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
            ...
            # Fallback robusto (es. "Jan 2025", "01/2025", "2025-01-15")
            dt = pd.to_datetime(mese_raw, errors="coerce", dayfirst=True)
            if pd.notna(dt):
                year, month = int(dt.year), int(dt.month)
            else:
                y_match = re.search(r"(20\d{2})", mese_raw)
                m_match = re.search(r"(?<!\d)([1-9]|1[0-2])(?!\d)", mese_raw)
                if y_match:
